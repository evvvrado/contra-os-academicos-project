        ...
        <div class="card">
            <div class="card-body">
                ...
                <form id="form-cadastro" action="{{route('painel.acessorios.salvar')}}" method="POST">
                    @csrf
                    <input type="hidden" name="acessorio_id" value="{{ $acessorio->id }}">
                    <div class="col-lx-12">
                        <div class="row">
                            <div class="form-group col-6 col-lg-6 mt-3">
                                <label for="nome">Nome</label>
                                <input required id="nome" name="nome" type="text" placeholder="Insira o nome do acessorio" value="{{ $acessorio->nome }}" class="form-control">
                            </div>
                            ...
                            <div class="form-group col-6 col-lg-6 mt-3">
                                ...
                                <select class="form-control" name="acessorio_categoria_id" required>
                                    ...
                                    @foreach($categorias as $categoria)
                                        <option value="{{$categoria->id}}" @if($acessorio->acessorio_categoria_id == $categoria->id) selected @endif>{{$categoria->nome}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-6 col-lg-6 mt-3">
                                ...
                                <select class="form-control" name="validade" required>
                                    @foreach(config("ingredientes.validades") as $key => $validade)
                                        <option value="{{ $key }}" @if($acessorio->validade == $key) selected @endif>{{ $validade }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
