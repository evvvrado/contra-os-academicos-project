            ...
            <div class="niv-content">
                ...
                @if ($orcamento->produtos->count() > 0)
                    ...
                    @foreach ($orcamento->orcamento_produtos->groupBy("produto_id") as $produto_id => $orcamento_produtos)
                        ...
                        <div class="box " niv-fade data-cal="{{ $orcamento_produtos->first()->produto->calorias }}"
                            data-teor="{{ $orcamento_produtos->first()->produto->teor_alcoolico }}" {{ $lancamento }}>
                            <picture>
                                <img src="{{ asset($orcamento_produtos->first()->produto->imagem_preview) }}" alt="imagem representativa"
                                    style="width: auto; height: 100%; margin: auto;">
                            </picture>
                            ...
                            <span>
                                ...
                                <p>{{ mb_strimwidth($orcamento_produtos->first()->produto->descricao, 0, 72, '...') }}</p>
                                <hr>
                                <h5 style="margin-top: 15px;">Variações:</h5>
                                @foreach($orcamento_produtos as $orcamento_produto)
                                    <p>{{ $orcamento_produto->receita->nome }}</p>
                                @endforeach
                                {{-- <main>
                                    <div>
                                        <strong>Teor alcóolico</strong>
                        ...
                        <span>
                            ...
                            <main>
                                <form wire:submit.prevent>
                                    ...
                                    <button class="remove" wire:click="selecionar({{ $receita->produto_id }})">+</button>
                                </form>
                            </main>
                        </span>
                    </div>
